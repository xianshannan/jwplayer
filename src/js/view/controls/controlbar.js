...
function buttonsInFirstNotInSecond(buttonsA, buttonsB) {
  return buttonsA.filter(
    a =>
      !buttonsB.some(
        b =>
          b.id + b.btnClass === a.id + a.btnClass && a.callback === b.callback
      )
  );
}
...
export default class Controlbar {
  constructor(_api, _model) {
    ...
    _model.change('originalDuration', this.onDuration, this);
    ...
    this._model.mediaController.on(
      'seeked',
      function() {
        _model.once('change:position', this.checkDvrLiveEdge, this);
      },
  ...
  onElapsed(model, val) {
    ...
    if (model.get('streamType') === 'DVR') {
      elapsedTime = countdownTime =
        val >= dvrSeekLimit ? '' : '-' + utils.timeFormat(-val);
    } else {
  ...
  rewind() {
    const currentPosition =
      this._model.get('timelinePostion') || this._model.get('postion');
    ...
    const videoPosition = this.elements.time.renderByPercent(
      rewindPosition,
      'click'
    );
    // Seek 10s back. Seek value should be >= 0 in VOD mode and >= (negative) duration in DVR mode
    this._api.seek(Math.max(videoPosition, startPosition), reasonInteraction());
  }
